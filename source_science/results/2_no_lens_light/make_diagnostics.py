"""
Diagnostic plots for test 2 (no lens light).

Same outputs as `1_with_lens_light/make_diagnostics.py` but for the two
no-lens-light fits.
"""
...
MODEL_NAMES = ("sersic_source", "mge_source")
MODELS_WITH_LINEAR_LP = {"mge_source"}
MODEL_COLORS = {"sersic_source": "tab:blue", "mge_source": "tab:red"}
MODEL_LABELS = {"sersic_source": "Sersic source", "mge_source": "MGE source"}
...
def main():
    ...
    ax.set_title("Test 2 — Source radial profile (no lens light)")
    ...
    ax.set_title("Test 2 — Cumulative source flux (no lens light)")
    ...
    extent = 0.5 * panels[0][1].shape[0] * 0.005
    ...
    fig.suptitle("Test 2 — Source-plane brightness (no lens light)", fontsize=12)
