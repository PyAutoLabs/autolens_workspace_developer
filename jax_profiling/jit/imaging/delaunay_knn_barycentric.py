"""
JAX Profiling: KNN-Barycentric Imaging Likelihood (Step-by-Step)
================================================================

Parallel of ``delaunay.py`` with the source-plane mesh swapped from
``al.mesh.Delaunay`` (scipy.spatial.Delaunay triangulation + barycentric
weights via a CPU ``pure_callback``) to ``al.mesh.KNNBarycentric`` (brute-
force k-nearest-neighbour search in JAX with k=3 + locally-exact barycentric
weights on the triangle formed by those 3 nearest mesh vertices).

NEGATIVE RESULT (documented here so the record survives)
--------------------------------------------------------

At the HST fiducial used here (39x39 overlay → 1231 vertices, MGE-60 lens):

- ``log_evidence`` drifts from Delaunay by ~2.2% (584 nats higher).
- ~5% of mesh vertices (60/1291) are *never* in any query's nearest-3 nor
  any split-point's nearest-3 — they're paid for but never used. This makes
  the regularization matrix ``H`` structurally rank-deficient by 60.
- Per the decision tree in ``PyAutoPrompt/issued/knn_barycentric.md``,
  this fails the most lenient gate (``rtol=1e-2``) and falls under
  "abandon; fall back to split-callback approach in
  PyAutoPrompt/issued/delaunay_research.md".

This script remains in the repo as documentary evidence — it pins the
observed KNNBarycentric ``log_evidence`` (26872.33) at ``rtol=1e-4`` as a
regression catch, separately reports the relative error vs Delaunay's
26288.32, and prints a science-gate verdict block at the bottom.

Pipeline identical to ``delaunay.py``: same dataset, same model, same
ConstantSplit regularization, same step-by-step JIT breakdown.

Pipeline steps (unchanged from delaunay.py):
...
6. KNN-barycentric interpolation + mapper (replaces scipy Delaunay callback)
7. Mapping matrix
...
with timer.section("knn_barycentric_interpolation_and_mapper"):
    # Polymorphic instantiation via mesh.interpolator_cls — KNNBarycentric mesh
    # returns InterpolatorKNNBarycentric here without needing an explicit import.
    interpolator = pixelization_obj.mesh.interpolator_cls(
...
# NOTE: the rtol=1e-4 self-consistency assertion present in ``delaunay.py`` is
# intentionally NOT run here. The manual ``compute_log_evidence`` above takes a
# naive ``slogdet(H)`` of the regularization matrix. For Delaunay this is
# finite because scipy.spatial.Delaunay guarantees every mesh vertex belongs to
# at least one simplex → every vertex appears in at least one query's mapping
# → H is full-rank (modulo the 1-dim constant null-space of a Laplacian).
#
# kNN-barycentric makes no such guarantee: with k=3, some mesh vertices
# (especially in sparse edge regions) are simply never among any query's top-3
# nor any split-point's top-3. Those vertices contribute identically-zero
# rows/columns to H, making H structurally rank-deficient — at the HST
# fiducial here, ~60/1291 (~5%) of mesh vertices are unused, so
# ``slogdet(H) = -inf`` and the manual path returns ``-inf``.
#
# ``FitImaging.log_evidence`` (the production path) handles this gracefully
# via ``log_det_curvature_reg_matrix_term`` which filters/regularizes the
# zero-eigenvalue modes. So the FitImaging-reference value above is the
# trustworthy science answer, and the science-gate assertions at the bottom
# of the script run against it.
unused_mesh_vertices = int(
    (np.linalg.eigvalsh(np.asarray(inversion.regularization_matrix)) <= 1e-12).sum()
)
print(
    f"  H rank deficiency: {unused_mesh_vertices}/{inversion.regularization_matrix.shape[0]} "
    f"mesh vertices unused by the kNN-barycentric interpolator "
    f"({100 * unused_mesh_vertices / inversion.regularization_matrix.shape[0]:.1f}%)"
)
...
results_dir = _workspace_root / "jax_profiling" / "results" / "jit" / "imaging"
results_dir.mkdir(parents=True, exist_ok=True)

dict_path = results_dir / f"delaunay_knn_barycentric_likelihood_summary_{instrument}_v{al_version}.json"
...
chart_path = results_dir / f"delaunay_knn_barycentric_likelihood_summary_{instrument}_v{al_version}.png"
...
# ===================================================================
# Regression assertion & science-gate verdict
# ===================================================================
#
# This script's reason for existing is the science gate from the wildcard
# investigation in PyAutoArray PR #318. The verdict at the HST fiducial is
# **NEGATIVE**: KNNBarycentric's log_evidence drifts from Delaunay's by
# ~2.2%, well outside the rtol=1e-3 ship-as-opt-in gate and even outside
# rtol=1e-2 (abandon threshold).
#
# Per the decision tree in PyAutoPrompt/issued/knn_barycentric.md:
# rtol=1e-2 fail → abandon; fall back to split-callback approach in
# PyAutoPrompt/issued/delaunay_research.md.
#
# The assertion below is therefore a regression CATCH for KNNBarycentric's
# own implementation (pinned to the observed value at rtol=1e-4) — not the
# science gate. The science gate is a one-shot computation below, reported
# but not asserted, so future invocations of the script remain informative
# even though the verdict is already settled.

EXPECTED_KNNBARY_LOG_EVIDENCE_HST = 26872.328684558823  # observed 2026-05-16
DELAUNAY_HST_REFERENCE = 26288.321397232066  # Delaunay value at same fiducial (from delaunay.py)

# Regression catch: drift in KNNBarycentric's own implementation
np.testing.assert_allclose(
    log_evidence_ref,
    EXPECTED_KNNBARY_LOG_EVIDENCE_HST,
    rtol=1e-4,
    err_msg=(
        f"imaging/delaunay_knn_barycentric[{instrument}]: regression — eager log_evidence "
        f"drifted from KNNBarycentric's pinned value at rtol=1e-4 "
        f"(got {log_evidence_ref}, expected {EXPECTED_KNNBARY_LOG_EVIDENCE_HST}). "
        f"Either KNNBarycentric implementation changed or the dataset/model changed."
    ),
)
print(
    f"  Eager regression assertion PASSED: KNNBarycentric log_evidence "
    f"{log_evidence_ref:.6f} matches pinned {EXPECTED_KNNBARY_LOG_EVIDENCE_HST:.6f} "
    f"at rtol=1e-4"
)
np.testing.assert_allclose(
    float(full_result),
    ...
    rtol=1e-4,
    err_msg=(
        f"imaging/delaunay_knn_barycentric[{instrument}]: regression — full-pipeline "
        f"JIT log_evidence drifted from KNNBarycentric's pinned value at rtol=1e-4"
    ),
)
if run_vmap:
    np.testing.assert_allclose(
        ...
        rtol=1e-4,
        err_msg=(
            f"imaging/delaunay_knn_barycentric[{instrument}]: regression — vmap "
            f"log_evidence drifted from KNNBarycentric's pinned value at rtol=1e-4"
        ),
    )

# ---------------------------------------------------------------------------
# Science-gate verdict (reported, not asserted)
# ---------------------------------------------------------------------------
_delaunay_rel_err = abs(float(log_evidence_ref) - DELAUNAY_HST_REFERENCE) / abs(
    DELAUNAY_HST_REFERENCE
)
if _delaunay_rel_err <= 1e-4:
    _verdict = "PASSES rtol=1e-4 gate → Delaunay-equivalent (ship as drop-in default)"
elif _delaunay_rel_err <= 1e-3:
    _verdict = "PASSES rtol=1e-3 gate → Delaunay-class approximation (ship as opt-in alternative)"
elif _delaunay_rel_err <= 1e-2:
    _verdict = "PASSES rtol=1e-2 gate → marginal, decide case-by-case"
else:
    _verdict = (
        "FAILS rtol=1e-2 gate → abandon; fall back to split-callback approach "
        "in PyAutoPrompt/issued/delaunay_research.md"
    )

print("")
print("=" * 70)
print("SCIENCE GATE VERDICT (KNN-barycentric vs Delaunay at HST fiducial)")
print("=" * 70)
print(f"  KNNBarycentric log_evidence: {log_evidence_ref:.6f}")
print(f"  Delaunay log_evidence:       {DELAUNAY_HST_REFERENCE:.6f}")
print(f"  Absolute difference:         {float(log_evidence_ref) - DELAUNAY_HST_REFERENCE:+.4f}")
print(f"  Relative error:              {_delaunay_rel_err:.4e}")
