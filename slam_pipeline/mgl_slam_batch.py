"""
SLaM (Source, Light and Mass): Multi-Galaxy Lens (MGL)
======================================================

This script uses the SLaM pipelines to fit a multi-galaxy strong lens (MGL), including extra galaxies
and scaling galaxies surrounding the main lens whose light and mass are both modeled.

__Contents__

**Prerequisites:** Before using this SLaM pipeline, you should be familiar with.
**Extra Galaxies and Scaling Galaxies:** This MGL SLaM pipeline handles two distinct categories of companion galaxies, which differ in.
...
**SLaM Pipeline:** Overview of the SLaM pipeline for this example.
...
- **Multi-Galaxy Lens (MGL)** (`mgl/modeling`):
    How we model multi-galaxy strong lenses in PyAutoLens, including how we include
    extra galaxies in the lens model.

__Extra Galaxies and Scaling Galaxies__

This MGL SLaM pipeline handles two distinct categories of companion galaxies, which differ in
...
Einstein radius prior is bounded by a value derived from the galaxy's luminosity:
    ...
    upper_limit = min(2 * (einstein_radius_bgc / L_bgc^0.5) * L^0.5, 5.0)
...
shared scaling relation. The MGL pipeline introduced here is appropriate when:
...
and TOTAL MASS PIPELINE this SLaM modeling script fits `Imaging` data of a multi-galaxy strong lens
...
def dataset_instrument_hdu_dict_via_fits_from(
    ...
    """
    with fits.open(dataset_path / dataset_fits_name) as hdu_list:
        instrument_dict = {}
        counter = 0
...
(`min(2 * (einstein_radius_bgc / L_bgc^0.5) * L^0.5, 5.0)`). Scaling galaxies use a Faber-Jackson relation anchored to the
BGC: `einstein_radius = einstein_radius_bgc * (luminosity / luminosity_bgc)^0.5`, where
`einstein_radius_bgc` is the free Isothermal Einstein radius of the brightest main lens.
"""
...
def source_lp_1(
    ...
    # Computed here so luminosity_bgc is available for both the extra galaxy bounds
    # and the scaling galaxy Faber-Jackson relation below.
    main_luminosities = [
        np.sum([
    ...
    # Upper bound is calibrated from the BGC: 2 * (upper_einstein_radius / L_bgc^0.5) * L^0.5.
    # upper_einstein_radius serves as a proxy for the BGC Einstein radius at this stage
    # since the main lens masses are introduced here for the first time.
    # The factor of 2 makes the bound conservative.
    ...
    for i in range(n_extra):
        ...
        mass.einstein_radius = af.UniformPrior(
            lower_limit=0.0,
            upper_limit=min(
                2 * (upper_einstein_radius / luminosity_bgc**0.5) * total_luminosity**0.5,
                5.0,
            ),
        )
    ...
    # --- scaling lens galaxy models (Faber-Jackson scaling anchored to BGC) ---
    # einstein_radius = einstein_radius_bgc * (luminosity / luminosity_bgc)^0.5
    # einstein_radius_bgc is the free Isothermal Einstein radius of the brightest main lens.
    scaling_mass_models = []
    for i in range(n_scaling):
        ...
        mass.einstein_radius = (
            ...
            * (total_luminosity / luminosity_bgc) ** 0.5
        )
...
def light_lp(
    ...
    # The adapt images were built on the source_pix mask, which is smaller than this
    # dataset's mask when scaling galaxies enlarge it; remap them onto this dataset's
    # mask (the annulus gains zeros, correct since the source has no light there).
    adapt_images = al.AdaptImages(
        galaxy_name_image_dict={
            name: al.Array2D(values=image.native, mask=dataset.mask)
            for name, image in adapt_images.galaxy_name_image_dict.items()
        },
        galaxy_name_image_plane_mesh_grid_dict=adapt_images.galaxy_name_image_plane_mesh_grid_dict,
    )
...
use a Faber-Jackson relation anchored to the BGC PowerLaw Einstein radius, both paired with
their fixed `light[1]` bulge.
"""
...
def mass_total(
    ...
    # --- Find BGC: the brightest of the main lens galaxies ---
    # Computed here so luminosity_bgc and einstein_radius_bgc are available for both
    # the extra galaxy bounds and the scaling galaxy Faber-Jackson relation below.
    main_luminosities = [
        np.sum([
            2 * np.pi * g.sigma**2 / g.axis_ratio() * g.intensity
            for g in getattr(light_result.instance.galaxies, f"lens_{i}").bulge.profile_list
        ]) / pixel_scale**2
        for i in range(n_lenses)
    ]
    bgc_index = int(np.argmax(main_luminosities))
    luminosity_bgc = main_luminosities[bgc_index]
    bgc_key = f"lens_{bgc_index}"
    einstein_radius_bgc = getattr(
        source_pix_result_1.instance.galaxies, bgc_key
    ).mass.einstein_radius

    # --- extra galaxies: fixed light, free mass ---
    # Upper bound calibrated from the BGC's fitted Isothermal Einstein radius:
    #   2 * (einstein_radius_bgc / L_bgc^0.5) * L^0.5
    # This is the Faber-Jackson scaling prediction doubled to stay conservative.
    # Also capped at 2x the previous-stage Einstein radius of this galaxy.
    extra_mass_models = []
    for i in range(n_extra):
        ...
        mass.einstein_radius = af.UniformPrior(
            lower_limit=0.0,
            upper_limit=min(
                2 * (einstein_radius_bgc / luminosity_bgc**0.5) * total_luminosity**0.5,
                2 * light_extra.mass.einstein_radius,
            ),
        )
    ...
    # --- scaling galaxies: fixed light, Faber-Jackson scaling anchored to BGC ---
    # einstein_radius = einstein_radius_bgc * (luminosity / luminosity_bgc)^0.5
    # einstein_radius_bgc is the free PowerLaw Einstein radius of the brightest main lens.
    scaling_mass_models = []
    for i in range(n_scaling):
...
def _cli_arg(flag: str, default: str) -> str:
    """
    Return the value passed via `--<flag>=<value>` on the command line, falling back to
    `default` if no such argument was supplied (e.g. when running interactively).
    """
    prefix = f"--{flag}="
...
sample_name = _cli_arg("sample", default="euclid_dr1")
dataset_name = _cli_arg("dataset", default="102022474_NEG590266584471556814")
dataset_path = Path("dataset") / sample_name / dataset_name
...
for image_tag in ("_FLUX", "_BGSUB"):
    ...
    if dataset_index_dict:
        print(f"Using image tag: {image_tag}")
        break
else:
    raise ValueError(
        f"No HDUs ending in '_FLUX' or '_BGSUB' found in {dataset_name}.fits"
    )
